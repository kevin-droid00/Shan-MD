...
cmd({
    ...
    try {
        const prog = await fetchJson(`https://www.ominisave.com/api/ytmp3?url=${q}`)
        if (!prog || !prog.result.url) return await reply('*Conversion failed, try again!*');

        try {
            const bytes = await checkFileSize(prog.result.url, config.MAX_SIZE);
...
k.key } });
        ...
        await conn.sendMessage(
            from,
            { audio: { url: prog.result.url }, mimetype: 'audio/mpeg' },
            { quoted: mek }
        );
...
async (conn, mek, m, { from, q, reply }) => {
  ...
  try {
    const prog = await fetchJson(`https://www.ominisave.com/api/ytmp3?url=${encodeURIComponent(q)}`);
...
k.key } });
    ...
    const res = await fetch(prog.result.url);
...
async (conn, mek, m, { from, q, reply }) => {
    try {
        ...
        const prog = await fetchJson(`https://www.ominisave.com/api/ytmp3?url=${datae}`);
